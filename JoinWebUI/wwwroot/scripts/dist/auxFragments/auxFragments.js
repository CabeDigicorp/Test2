...
export class AuxFragments {
    constructor(world) {
        ...
        Object.defineProperty(this, "transparentFragments", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {}
        });
        Object.defineProperty(this, "highlightedFragments", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
     ...
     * @returns
     */
    createTransparentFragment(baseFragment, modelPos, opacityLevel) {
        if (!this.transparentFragments[opacityLevel]) {
            this.transparentFragments[opacityLevel] = new Map();
        }
        //create transparent materials for fragment
        ...
        for (const iid of baseFragment.ids) {
            ...
            let tItem = { id: item.id, transforms: [], colors: [] };
            //tItem.id = item.id;
            //tItem.transforms = [];
            for (const m of item.transforms) {
                tItem.transforms[tItem.transforms.length] = m.clone();
     ...
     * @returns
     */
    copyToTransparentMaterial(material, opacityLevel) {
     ...
     */
    setTransparencyByIdMap(opacityLevel, fragmentIdMap, transparent) {
        if (!this.transparentFragments[opacityLevel]) {
    ...
    disposeAllOpacityLevels(disposer) {
 ...
 * @returns
 */
    createHighlightedFragment(baseFragment, modelPos) {
        if (!this.highlightedFragments) {
            this.highlightedFragments = new Map();
        }
        //create transparent materials for fragment
        let highlightedMaterials = new Array();
        for (const material of baseFragment.mesh.material) {
            highlightedMaterials[highlightedMaterials.length] = this.copyToHighlightingMaterial(material);
        }
        //create new transparent fragment
        ...
        hFrag.add(highlightedItems);
        hFrag.mesh.renderOrder = Infinity;
        hFrag.mesh.frustumCulled = false;
        if (modelPos.x != 0 || modelPos.y != 0 || modelPos.z != 0) {
            hFrag.mesh.translateX(modelPos.x);
            hFrag.mesh.translateY(modelPos.y);
            hFrag.mesh.translateZ(modelPos.z);
        }
        //finalize creation
        hFrag.setVisibility(false);
     ...
     * @returns
     */
    copyToHighlightingMaterial(material) {
        let highlightingMaterial = new THREE.MeshLambertMaterial();
 ...
 */
    setHighlightingByIdMap(fragmentIdMap, highlighted) {
        if (!this.highlightedFragments) {
...
//# sourceMappingURL=auxFragments.js.map