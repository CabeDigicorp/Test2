...
namespace JoinLicenseService
{
    public partial class ActivateMachine : System.Web.UI.Page
    {
        ...
        internal const string exceptionPrefix = "##exception:";
        ...
        //test ale
        protected void btnDisableLicense_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string newLicenseCode = srvc.DisableLicense(txtLicenseCode.Text, txtMachineCode.Text, settingsFilePath);

                if (newLicenseCode.StartsWith(exceptionPrefix))
                    throw new Exception(newLicenseCode.Substring(exceptionPrefix.Length));

                string ret = srvc.IsLicenseDisabled(txtLicenseCode.Text, settingsFilePath);

                lblMessage.Text = "The license has been disabled <br><br>" +
                    newLicenseCode;
                lblMessage.ForeColor = Color.Empty;
            }
            catch (Exception ex)
