...
namespace DevZest.Windows.DataVirtualization
{
    ...
    public partial class VirtualList<T> : IDisposable, IVirtualList, IList<VirtualListItem<T>>, INotifyPropertyChanged, INotifyCollectionChanged
    {
        ...
        public bool IsPopulating {get;set; }//by ale

        public bool IsLoading { get => LoadingState != QueuedBackgroundWorkerState.Processing; }//by Ale
        ...
        {
            IsPopulating = true;//by ale

            if (overallCount > 0 && startIndex >= overallCount)//by ale
                return;
