        ...
        public class RtfTreeNode
        {
            ...
            private string GetText(bool raw, int ignoreNchars)
            {
                ...
                if (NodeType == RtfNodeType.Group)
                {
                    ...
                    if (raw ||
                       (!ChildNodes[indkw].NodeKey.Equals("fonttbl") &&
                        ...
                        !ChildNodes[indkw].NodeKey.Equals("colorschememapping") && //add by Ale x devexpress
                        !ChildNodes[indkw].NodeKey.Equals("themedata") && //Add by Ale x devexpress
                        !ChildNodes[indkw].NodeKey.Equals("bkmkstart") && //Add by Ale x devexpress
                        !ChildNodes[indkw].NodeKey.Equals("bkmkend"))) //Add by Ale x devexpress
                    {
                        if (ChildNodes != null)
                        {
                            int uc = ignoreNchars;
                ...
                else if (NodeType == RtfNodeType.Control)
                {
                    if (NodeKey == "'")
                    {
                        //rem by ale 06/03/2024  
                        //commentato perchè dovrebbe esserci stato affianco anche il carattere unicode
                        //esempio \u232\'e8
                ...
                else if (NodeType == RtfNodeType.Keyword)
                {
                    ...
                    else if (NodeKey.Equals("u"))
                    {
                        ...
                        if ((Parameter >= 0 && Parameter < 0x0000D800) || (Parameter > 0x0000DFFF && Parameter < 0x0010FFFF))
                            s = Char.ConvertFromUtf32(Parameter);
                        else
                            s = DecodeControlChar(Parameter, tree.GetEncoding());
